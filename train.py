...
from disen import Image_adapter
import diffusers
...
def parse_args(input_args=None):
    ...
    #         action="store_true",
    #         help="Enable hybrid attention mechanism"
    #     )
    #
                    ...
                    for image, mask in zip(self.instance_images, self.instance_masks):
                        ...
                        for interesting_are in [1]:
                            inpaint_work = inpaint_worker.InpaintWorker(
                                ...
                                use_fill=True,
                                k=interesting_are,
                                path_upscale_models="./models/upscale_model/fooocus_upscaler_s409985e5.bin",
                            )
...
def main(args):
    ...
    # ������������������ image_adapter
    #image_adapter = Image_adapter(hidden_size=1024, num_attention_heads=8, num_layers=2).to(accelerator.device)
        ...
        for step, batch in enumerate(train_dataloader):
            with (accelerator.accumulate(unet)):
                ...
                if not args.train_text_encoder:
                    ...
                    # ============== �������������� image_adapter �� UNet ������������������ ==============
                    adapter_output = image_adapter(model_pred)
                    model_pred = model_pred + adapter_output
                ...
                # Define VGG Feature Extractor class
                ...
                # Instantiate VGG feature extractor
                vgg_feature_extractor = VGGFeatureExtractor().to(accelerator.device)  # Make sure to use your device
                ...
                def contrastive_loss(embedding1, embedding2, label, margin=1.0):
                    ...
                    loss_negative = (1 - label) * torch.pow(F.relu(margin - distance), 2)

                    loss = torch.mean(loss_positive + loss_negative) / 2
                    return loss

                def region_specific_semantic_loss(model_pred, target, region_size=(64, 64), stride=32):
                    """
                    Compute the Region-Specific Semantic Loss between the model prediction and target image.
                    - model_pred: The predicted image (output from the model)
                    - target: The target (ground truth) image
                    - region_size: The size of the region to consider for semantic similarity
                    - stride: The stride for sliding the window (for extracting regions)
                    """
                    # Assuming that both model_pred and target are in shape (batch_size, channels, height, width)

                    # Extract regions from both model_pred and target
                    batch_size, channels, height, width = model_pred.shape

                    # Initialize loss
                    total_loss = 0.0

                    # Loop over the image in sliding window fashion
                    for i in range(0, height - region_size[0] + 1, stride):
                        for j in range(0, width - region_size[1] + 1, stride):
                            # Extract regions
                            model_pred_region = model_pred[:, :, i:i + region_size[0], j:j + region_size[1]]
                            target_region = target[:, :, i:i + region_size[0], j:j + region_size[1]]

                            # Flatten the regions to vectors (for easier comparison)
                            model_pred_region = model_pred_region.view(batch_size, -1)  # Flatten each region
                            target_region = target_region.view(batch_size, -1)  # Flatten each region

                            # Compute cosine similarity between the two regions
                            cos_sim = F.cosine_similarity(model_pred_region, target_region, dim=-1)

                            # Compute loss: We want the cosine similarity to be as high as possible
                            region_loss = 1 - cos_sim  # 0 means perfect match, 1 means completely dissimilar
                            total_loss += torch.mean(region_loss)  # Average loss over the batch

                    return total_loss

                def position_aware_content_loss(model_pred, target, vgg_feature_extractor,
                                                layers=('relu1_2', 'relu2_2', 'relu3_3', 'relu4_3')):
                    """
                    Compute the Position-Aware Content Loss (PACL) between model_pred and target.
                    This loss enhances content consistency by considering the spatial position of different regions in the image.
                    - model_pred: The generated image (output from the model)
                    - target: The target image (ground truth)
                    - vgg_feature_extractor: A pretrained VGG feature extractor
                    - layers: List of VGG layers to use for content loss calculation
                    """
                    # Extract VGG features for both the model_pred and target
                    ...
                    # Loop over all the selected layers to compute content loss
                    for pred_feat, target_feat in zip(model_pred_features, target_features):
                        # Compute content loss (e.g., L2 loss between feature maps)
                        content_loss = F.mse_loss(pred_feat, target_feat)

                        # Apply position-awareness (weighting by spatial position)
                        height, width = pred_feat.shape[2], pred_feat.shape[3]
                        grid_x, grid_y = torch.meshgrid(torch.arange(height), torch.arange(width))
                        grid = torch.stack([grid_x, grid_y], dim=0).float().to(pred_feat.device)

                        # Normalize the grid
                        grid /= torch.max(grid)

                        # Apply a spatially aware weighting factor to the content loss
                        weighted_loss = content_loss * grid.sum()
                ...
                if args.with_prior_preservation:
                    ...
                    loss = loss + args.prior_loss_weight * prior_loss + contrastive_loss(inpaint_head_feature, prompt_embeds)

                    # Compute the Region-Specific Semantic Loss (RSSL)
                    rssl_loss = region_specific_semantic_loss(model_pred, prompt_embeds)

                    # Compute the Position-Aware Content Loss (PACL)
                    pacl_loss = position_aware_content_loss(model_pred, prompt_embeds, vgg_feature_extractor)

                    # Add RSSL and PACL to the total loss
                    loss += 1 * rssl_loss + 10 * pacl_loss  # You can scale these depending on your needs

                # if args.with_prior_preservation:
                #     # Add the prior loss to the instance loss.
                #     loss = loss + args.prior_loss_weight * prior_loss + contrastive_loss(model_pred.float(), target.float())
